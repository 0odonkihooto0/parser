...
> **Парсинг загруженных файлов.** Документы отправляются в Firecrawl напрямую
> через эндпоинт `/parse`. Если ваш локальный образ Firecrawl устарел и не
> поддерживает `/parse` (ошибка `Cannot POST /v2/parse`), приложение
> автоматически переключается на запасной путь: отдаёт файл по внутреннему
> адресу (`INTERNAL_BASE_URL`), и Firecrawl скачивает его сам. Для наилучшего
> качества обновите образ: `docker compose -f docker-compose.selfhosted.yml pull`.
