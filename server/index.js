...
app.post('/api/scrape', async (req, res) => {
  ...
  try {
    ...
    if (mode === 'scrape') {
      ...
      const result = await firecrawl.scrapeUrl(url, { formats: ['markdown'], timeout: 180000, parsePDF: pdfMode });
      markdown = result.markdown ?? '';
      rawMeta = result.metadata ?? null;
...
app.post('/api/upload', upload.single('file'), async (req, res) => {
  ...
  try {
    const result = await firecrawl.scrapeUrl(fileUrl, { formats: ['markdown'], timeout: 180000, parsePDF: pdfMode });
